...
module.exports = function () {
    prompt().bind({}).then(function (result) {
        ...
        } else if (this.input.format === 'csv'){
            return write.csv(this.input.output, restaurants);
        } else if (this.input.format === 'tsv'){
            return write.tsv(this.input.output, restaurants);
        } else {
            console.log(restaurants);
            return null;
        }
    }).then(function (file) {
        if(file) console.log("Scraped results are successfully written to file: "+file+"."+this.input.format);
